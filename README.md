...
3. IP adresleri bazında oransal bir **"Tehdit Skoru"** hesaplayarak şüpheli aktörleri tespit eder.
4. **Zaman Serisi Analizi (Time-Series)** ile 5 dakikalık pencerelerde hız analizi yaparak **Brute Force (Kaba Kuvvet) ve DDoS** saldırılarını anında yakalar ve raporlar.
...
3. **Zaman Serisi ve Hız Analizi:** Pandas'ın `Grouper(freq='5min')` özelliği ile log kayıtları zamansal pencerelere oturtulmuş; bu sayede ardışık ve yoğun istek atan saldırganların tespiti için ekstra bir kontrol (Control) katmanı oluşturulmuştur.
4. **Hata Yönetimi (Exception Handling):** `try/except` kullanılarak dosya bulunamama (FileNotFoundError) durumlarında kodun çökmesi engellenmiş ve otomatik simülasyon başlatma yeteneği eklenmiştir.
5. **DRY (Kendini Tekrar Etme):** 1000 satırlık log verisi tek tek elle yazılmak yerine, döngüler ve rastgele seçim (random.choices ağırlıklandırma) mantığıyla dinamik olarak üretilmiştir.
